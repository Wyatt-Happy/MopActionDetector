...
from core.utils.utils import validate_video_path, generate_unique_id
...
class EmbeddingDBManager:
    ...
    def add_video_embeddings(
        ...
        if action_type not in [self.config.ACTION_MOPPING, self.config.ACTION_NON_MOPPING]:
            logger.error(f"无效动作类型：{action_type}")
            return 0
        ...
        success_count = 0
        logger.info(f"开始入库 {action_type} 视频（共{len(video_paths)}个）")

        # 2. 逐个提取并入库
        for idx, video_path in enumerate(video_paths):
            logger.info(f"处理 [{idx + 1}/{len(video_paths)}]: {video_path}")
            # 提取向量
            embedding = self.feature_extractor.extract_video_embedding(video_path)
            if embedding is None:
                logger.warning(f"跳过：{video_path} 向量提取失败")
                continue
            ...
            try:
                ...
                logger.info(f"入库成功（ID：{vec_id}）")
                success_count += 1
            except Exception as e:
                logger.error(f"入库失败：{type(e).__name__} - {e}")
                traceback.print_exc()

        logger.info(f"入库完成：成功{success_count}/{len(video_paths)}个")
    ...
    def delete_embeddings(self, action_type: Optional[str] = None):
        ...
        if action_type is None:
            ...
            try:
                ...
                logger.info(f"已清空所有向量数据（共删除{len(all_ids)}条）")
            except Exception as e:
        ...
        # 2. 删除指定类型数据（原有逻辑，兼容新版）
            ...
            try:
                ...
                logger.info(f"已清空 {action_type} 类型向量数据")
            except Exception as e:
                logger.error(f"清空{action_type}类型数据失败：{e}")
                traceback.print_exc()

    def check_db_status(self) -> int:
        """查看数据库状态（统计数据量，彻底修复numpy真值判断问题）"""
        total_count = self.collection.count()
        logger.info(f"向量库状态 - 总数据量：{total_count}")
        ...
        if total_count > 0:
            ...
            # 打印统计结果
            logger.info(f"  - 拖地视频数量：{mop_count}")
            logger.info(f"  - 非拖地视频数量：{non_mop_count}")
            logger.info(f"  - 无效元数据数量（无action_type）：{invalid_meta_count}")
            logger.info(f"  - 向量维度：{self.config.EMBEDDING_DIM}（配置默认）")

            # 提示清理无效数据
            if invalid_meta_count > 0:
                logger.warning(f"发现 {invalid_meta_count} 条无效数据，建议执行clean_invalid_data()清理！")

        return total_count

    def clean_invalid_data(self):
        """清理向量库中无action_type的无效数据（稳定版）"""
        logger.info("开始清理无效数据（无action_type）")
        total_count = self.collection.count()
        if total_count == 0:
            logger.info("向量库为空，无需清理")
            return
        ...
        if len(invalid_ids) > 0:
            ...
            logger.warning(f"发现 {len(invalid_ids)} 条无效数据，ID示例：{show_ids}...")
            # 二次确认
            confirm = input("⚠️ 确认删除这些无效数据吗？(y/n)：")
            if confirm.lower() == "y":
                self.collection.delete(ids=invalid_ids)  # 用ID删除无效数据
                logger.info(f"已删除 {len(invalid_ids)} 条无效数据")
            else:
                logger.info("取消删除操作")
        else:
            logger.info("未发现无效数据")
