...
try:
    ...
    for video in mopping_videos:
        if os.path.exists(video):
            success = detector.add_training_video("mopping", video)
    ...
    for video in phone_videos:
        if os.path.exists(video):
            success = detector.add_training_video("phone", video)
    ...
    if os.path.exists(test_video):
        ...
        # 模拟 UI 调用的接口
        print("\n模拟 UI 接口调用:")
        result = detector.detect(test_video, return_all=True)
        print(f"  接口返回: {result}")
        ...
        if 'all_results' in result:
            ...
            for item in result['all_results']:
                print(f"  {item['behavior_name']}: {item['similarity'] * 100:.1f}%")
    else:
        print(f"测试视频不存在: {test_video}")
    
    # 检查数据库状态
    print("\n数据库状态:")
    stats = detector.get_behavior_stats()
    print(f"  行为数量: {stats['total_behaviors']}")
    for behavior in stats['behaviors']:
        print(f"  {behavior['behavior_id']}: {behavior['video_count']} 个视频")
